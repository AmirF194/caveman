/** Reproducible F16 candidate producer. The shared support ledger is untouched. */
...
const directory = ['examples/middleware/mastra/certification', suffix].filter(Boolean).join('/');
...
await new Promise((yes, no) => {
  const child = spawn(process.execPath, ['--test', '--test-reporter=tap', testFile], { cwd: root, env, stdio: ['ignore', 'pipe', 'pipe'] });
  let output = '';
  ...
  child.on('close', async code => {
    ...
    try {
      ...
      if (code !== 0) throw new Error(`Native F16 execution failed with exit ${code}; inspect ${rawOutput}`);
      yes(output);
