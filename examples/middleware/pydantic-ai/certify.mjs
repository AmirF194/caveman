/** Produce and independently replay exact F14 candidates; never promote support. */
...
process.env.CAVEMAN_MIDDLEWARE_CERT_FAMILY = 'F14';
process.env.CAVEMAN_MIDDLEWARE_TEST_FAMILY = 'pydantic-ai';
const helper = await import('./certification-evidence.mjs');
const directory = process.argv.find(value => value.startsWith('--output='))?.slice('--output='.length) ?? `examples/middleware/pydantic-ai/certification/python`;
assert(directory.startsWith('examples/middleware/pydantic-ai/certification/') &&
...
await new Promise((yes, no) => {
  ...
  child.on('close', async code => {
    clearTimeout(timer);
    try { await writeFile(resolve(root, rawOutput), output); if (code !== 0) throw new Error(`Native F14 ${language} failed (${code}); inspect ${rawOutput}`); yes(); }
...
process.stdout.write(`${JSON.stringify({ family: 'F14', language, backed: summary.cells.filter(cell => cell.backed).length, unbacked: summary.cells.filter(cell => !cell.backed).map(({ id, missing }) => ({ id, missing })) })}\n`);
