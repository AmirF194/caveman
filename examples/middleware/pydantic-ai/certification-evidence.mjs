/** Frozen inputs for native Python F14 operation tests. */
import assert from 'node:assert/strict';
import { execFile } from 'node:child_process';
import { promisify } from 'node:util';
import { readFile } from 'node:fs/promises';

export const testFile = 'examples/middleware/pydantic-ai/test_native.py';
...
export async function certificationInputs() {
  ...
  const fixture = await json('examples/middleware/pydantic-ai/certification-cells.json');
  assert.deepEqual(fixture.cells, requiredCells().filter(cell => cell.family === 'F14' && cell.language === 'python'));
  const sources = new Map();
  const add = (path, role) => sources.set(path, { path, role });
  for (const name of ['pydantic_ai', '_native', '_usage', '_versions', '__init__']) add(`packages/middleware/python/caveman_middleware/${name}.py`, 'adapter_source');
  for (const name of ['test_native.py', 'example.py', '_certification_native.py', '_certification_fixture.py', '_test_result.py', 'certification-evidence.mjs', 'certification-cells.json', 'certify.mjs', 'run_native.mjs', '_fixture.py'])
    add(`examples/middleware/pydantic-ai/${name}`, 'test_source');
  ...
  add('examples/middleware/pydantic-ai/requirements.lock', 'dependency_lock');
  ...
  for (const [index, line] of source.split('\n').entries()) {
    ...
    if (!match) continue;
    const name = `PydanticCertification.${match[1]}`;
    tests.push({ id: `${testFile}::${name}`, file: testFile, name, line: index + 1, declaration: `def ${match[1]}(`, sha256: sha256(source),
      ...
      rerun: { program: 'node', args: ['--test', '--test-reporter=tap', driver], cwd: '.', env: { CAVEMAN_MIDDLEWARE_TEST_FAMILY: 'pydantic-ai', CAVEMAN_MIDDLEWARE_TEST_PYTHON: '<exact locked pydantic-ai environment>' } } });
  }
  assert.equal(tests.length, 4);
  const python = process.env.CAVEMAN_MIDDLEWARE_TEST_PYTHON; assert.ok(python);
  const names = ["pydantic-ai","pydantic-ai-slim","openai","anthropic","httpx2","httpcore2","pydantic"];
  ...
  return captureCertificationInputs({ family: 'F14', language: 'python', sources: [...sources.values()], tests: { schema_version: 1, tests },
