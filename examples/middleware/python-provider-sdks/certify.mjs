/** Produce exact native candidates without editing the shared support inventory. */
...
const directory = ['examples/middleware/python-provider-sdks/certification', suffix, family].filter(Boolean).join('/');
...
await new Promise((yes, no) => {
  ...
  child.on('close', async code => {
    ...
    try {
      ...
      if (code !== 0) throw new Error(`Native ${family} Python execution failed (${code}); inspect ${rawOutput}`);
      yes();
...
process.stdout.write(`${JSON.stringify({ family, language: 'python', backed: summary.cells.filter(row => row.backed).length,
