/** Produce and independently replay exact F08 candidates; never promote support. */
...
process.env.CAVEMAN_MIDDLEWARE_CERT_FAMILY = 'F08';
process.env.CAVEMAN_MIDDLEWARE_TEST_FAMILY = 'agno';
const helper = await import('./certification-evidence.mjs');
const directory = process.argv.find(value => value.startsWith('--output='))?.slice('--output='.length) ?? `examples/middleware/agno/certification/python`;
assert(directory.startsWith('examples/middleware/agno/certification/') &&
...
await new Promise((yes, no) => {
  ...
  child.on('close', async code => {
    clearTimeout(timer);
    try { await writeFile(resolve(root, rawOutput), output); if (code !== 0) throw new Error(`Native F08 ${language} failed (${code}); inspect ${rawOutput}`); yes(); }
...
process.stdout.write(`${JSON.stringify({ family: 'F08', language, backed: summary.cells.filter(cell => cell.backed).length, unbacked: summary.cells.filter(cell => !cell.backed).map(({ id, missing }) => ({ id, missing })) })}\n`);
