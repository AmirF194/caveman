/** Reproducible F04 candidate producer. The shared support ledger is untouched. */
...
const directory = ['examples/middleware/ai-sdk/certification', suffix].filter(Boolean).join('/');
...
await new Promise((yes, no) => {
  ...
  child.on('close', async code => {
    ...
    try {
      ...
      if (code !== 0) throw new Error(`Native F04 execution failed with exit ${code}; inspect ${rawOutput}`);
      yes(output);
