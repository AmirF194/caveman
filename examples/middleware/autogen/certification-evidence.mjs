/** Exact installed AutoGen inputs and literal native test identities. */
...
export const testFile = 'examples/middleware/autogen/test_native.py';
export const driver = 'packages/middleware/conformance/python-framework.test.mjs';
export async function certificationInputs() {
  const fixture = await json('examples/middleware/autogen/certification-cells.json');
  assert.deepEqual(fixture.cells, requiredCells().filter(cell => cell.family === 'F11'));
  const sources = [];
  for (const name of ['test_native.py', '_certification_native.py', '_test_result.py', 'certification-evidence.mjs', 'certification-cells.json', 'certify.mjs'])
    sources.push({ path: `examples/middleware/autogen/${name}`, role: 'test_source' });
  for (const path of [driver, 'packages/middleware/conformance/evidence_runtime.py', 'packages/middleware/conformance/runtime-fixture.mjs']) sources.push({ path, role: 'test_source' });
  const source = await readFile(`${root}/${testFile}`, 'utf8'), tests = [];
  for (const [index, line] of source.split('\n').entries()) {
    const match = line.match(/^\s*async def (test_(?:openai|anthropic)_exact_operation_journeys)\(/);
    if (!match) continue;
    const name = `NativeAutoGenCertification.${match[1]}`;
    tests.push({ id: `${testFile}::${name}`, file: testFile, name, line: index + 1, declaration: `def ${match[1]}(`,
      ...
      rerun: { program: 'node', args: ['--test', '--test-reporter=tap', driver], cwd: '.',
        env: { CAVEMAN_MIDDLEWARE_TEST_FAMILY: 'autogen', CAVEMAN_MIDDLEWARE_TEST_PYTHON: '<exact locked AutoGen environment>' } } });
  }
  assert.equal(tests.length, 2);
  const python = process.env.CAVEMAN_MIDDLEWARE_TEST_PYTHON; assert.ok(python);
  const names = ['autogen-agentchat', 'autogen-core', 'autogen-ext', 'openai', 'anthropic', 'httpx', 'pydantic'];
  ...
  return captureNativeCertificationInputs({ family: 'F11', language: 'python', sources, tests: { schema_version: 1, tests },
