/** Capture and independently replay exact native AutoGen operation evidence. */
...
const directory = ['examples/middleware/autogen/certification', suffix].filter(Boolean).join('/');
const encode = value => `${JSON.stringify(value, null, 2)}\n`;
const save = (path, value) => writeFile(resolve(root, path), encode(value));
await mkdir(resolve(root, directory), { recursive: true });
const input = await certificationInputs(), rawOutput = `${directory}/native.tap`;
await save(`${directory}/inputs.json`, input);
const env = Object.fromEntries(Object.entries(process.env).filter(([name]) => !/(?:API_KEY|ACCESS_TOKEN|AUTH_TOKEN|CLIENT_SECRET|SECRET_ACCESS_KEY|SESSION_TOKEN|GOOGLE_APPLICATION_CREDENTIALS|AWS_PROFILE|CAVEMAN_MIDDLEWARE_PACKAGED_TEST)/i.test(name)));
env.CAVEMAN_MIDDLEWARE_TEST_FAMILY = 'autogen';
await new Promise((yes, no) => {
  const child = spawn(process.execPath, ['--test', '--test-reporter=tap', driver], { cwd: root, env, stdio: ['ignore', 'pipe', 'pipe'] });
  let output = ''; const capture = chunk => { output += chunk; if (output.length > 16 * 1024 * 1024) child.kill('SIGTERM'); };
  child.stdout.on('data', capture); child.stderr.on('data', capture);
  const timer = setTimeout(() => child.kill('SIGTERM'), 180000);
  child.on('error', error => { clearTimeout(timer); no(error); });
  child.on('close', async code => {
    clearTimeout(timer);
    try { await writeFile(resolve(root, rawOutput), output); if (code !== 0) throw new Error(`Native F11 failed with exit ${code}; ${rawOutput}`); yes(); } catch (error) { no(error); }
  });
});
...
process.stdout.write(`${JSON.stringify({ family: 'F11', language: 'python', backed: summary.cells.filter(row => row.backed).length, unbacked: summary.cells.filter(row => !row.backed).map(({ id, missing }) => ({ id, missing })) })}\n`);
