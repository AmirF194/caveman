/** Reproducible F10 candidate producer. The shared support ledger is untouched. */
...
const directory = ['examples/middleware/crewai/certification', suffix].filter(Boolean).join('/');
const encode = value => `${JSON.stringify(value, null, 2)}\n`;
const save = (path, value) => writeFile(resolve(root, path), encode(value));
await mkdir(resolve(root, directory), { recursive: true });
process.env.CAVEMAN_MIDDLEWARE_CERT_FAMILY = 'F10';
...
env.CAVEMAN_MIDDLEWARE_TEST_FAMILY = 'crewai';
await new Promise((yes, no) => {
  ...
  child.on('close', async code => {
    ...
    try {
      ...
      if (code !== 0) throw new Error(`Native F10 execution failed with exit ${code}; inspect ${rawOutput}`);
      yes(output);
