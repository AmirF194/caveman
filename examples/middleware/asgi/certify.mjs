/** Capture and independently replay exact native ASGI operation evidence. */
...
const directory = ['examples/middleware/asgi/certification', suffix].filter(Boolean).join('/');
...
env.CAVEMAN_MIDDLEWARE_TEST_FAMILY = 'asgi';
await new Promise((yes, no) => {
  ...
  child.on('close', async code => {
    clearTimeout(timer);
    try { await writeFile(resolve(root, rawOutput), output); if (code !== 0) throw new Error(`Native F12 failed with exit ${code}; ${rawOutput}`); yes(); } catch (error) { no(error); }
  });
});
...
process.stdout.write(`${JSON.stringify({ family: 'F12', language: 'python', backed: summary.cells.filter(row => row.backed).length, unbacked: summary.cells.filter(row => !row.backed).map(({ id, missing }) => ({ id, missing })) })}\n`);
