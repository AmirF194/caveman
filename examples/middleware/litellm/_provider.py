...
class Provider:
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def respond(self):
                try:
                    ...
                    if body.get("model") == "fixture-native-error":
                        return self.send_json({"error": {"type": "invalid_request_error", "message": "deterministic native input error"}}, 400)
                    if "failure" in body.get("model", ""):
                        return self.send_json({"error": {"type": "server_error", "message": "deterministic native failure"}}, 503)
                    ...
                    parent.stream_done.clear()
                    first_content = False
                    for index, event in enumerate(parent.events(result)):
                        self.wfile.write(event)
                        self.wfile.flush()
                        content_event = (b'"content":' in event or b'"tool_calls":' in event) if parent.protocol == "openai-chat" else (b'"type": "content_block_delta"' in event) if parent.protocol == "anthropic-messages" else (b'"type": "response.output_text.delta"' in event or b'"type": "response.function_call_arguments.delta"' in event)
                        if content_event and not first_content:
                            first_content = True
    ...
    def answer(self, body):
        ...
        if isinstance(source, list):
            source = "".join(part.get("text", "") for part in source)
        ...
        if "recover-1" in found:
            recovered = found["recover-1"]
            if isinstance(recovered, list):
                recovered = "".join(part.get("text", "") for part in recovered)
            assert json.loads(recovered)["text"] == SOURCE
        else:
            assert source == SOURCE
        structured = body.get("response_format") is not None or (body.get("output_config") or {}).get("format") is not None
        return json.dumps({"answer": FACT}) if structured else FACT, None
